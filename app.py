...
import os
import math # Import math for non-linear transformation
...
def load_and_preprocess_data():
    ...
    # --- NON-LINEAR TRANSFORMATION (Accuracy Enhancement) ---
    
    # 1. Non-linear penalty for high Glucose (critical above 140mg/dL, severe above 223mg/dL)
    # This increases the magnitude of the scaled glucose feature for high values.
    # The higher the value, the greater the penalty.
    
    # We use a threshold of 1.0 (1 standard deviation above the mean, roughly the start of risk zone)
    # to apply the exponential penalty, making high glucose much more dominant in KNN distance.
    X['avg_glucose_level'] = X['avg_glucose_level'].apply(lambda x: x * 1.0 + (max(0, x - 1.0)**2) if x > 0 else x)

    # 2. Non-linear penalty for high BMI (risk increases significantly above 30)
    X['bmi'] = X['bmi'].apply(lambda x: x * 1.0 + (max(0, x - 1.5)**2) if x > 1.5 else x)
    
    # 3. Custom Weighting for Binary/Categorical High-Risk Features (Hypertension, Heart Disease, Smoking)
    # The base scale is already 0/1, we multiply to increase their importance in distance calculation.
...
with col2:
    ...
    avg_glucose_level = st.number_input(
        ...
        help="Fasting plasma glucose test results. Clinically critical range starts around 140 mg/dL."
    )
    
    bmi = st.number_input(
        "BMI (Body Mass Index)", 
        min_value=10.0, max_value=60.0, value=25.0, step=0.1,
        help="Calculated as weight (kg) / height (m)^2. Obesity (BMI > 30) is a major risk factor."
    )
...
if st.button("Predict Stroke Risk", type="primary"):
    ...
    # --- APPLY NON-LINEAR AND CUSTOM WEIGHTING TO INPUT DATA ---
    
    # Non-linear penalty for high Glucose (making high glucose values dominate)
    input_df['avg_glucose_level'] = input_df['avg_glucose_level'].apply(lambda x: x * 1.0 + (max(0, x - 1.0)**2) if x > 0 else x)
    
    # Non-linear penalty for high BMI (making high BMI values dominate)
    ...
    age_risk_score = min(100, max(0, (age - 45) / (70 - 45) * 100)) # Risk starts later, max at 70
    glucose_risk_score = min(100, max(0, (avg_glucose_level - 140) / (223 - 140) * 100)) # Risk based on 140mg/dL threshold, max at 223mg/dL
    bmi_risk_score = min(100, max(0, (bmi - 25) / (35 - 25) * 100)) # Risk based on 25 threshold, max at 35 (obesity)
    heart_hyper_risk_score = (hypertension_val * 0.6 + heart_disease_val * 0.4) * 100 # Hypertension gets slightly more weight
